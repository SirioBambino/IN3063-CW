...
class Grid:
    ...
    def display_grid(self, current_cell):
        ...
        cell_colour[current_cell] = 0.94, 0.79, 0.24
    ...
    def find_path(self):
        ...
        while current_cell != self.end_cell:
            ...
            for neighbour in neighbours:
                ...
                # Assign the next cell and the neighbour scores using their distance to the end and their value
                next_cell_score = math.dist(next_cell, self.end_cell) - (10 - self.grid[next_cell]) / 3
                neighbour_score = math.dist(neighbour, self.end_cell) - (10 - self.grid[neighbour]) / 3

                # If the neighbour's score is lower than the next cell score and the neighbour is not the
                # previous cell, make the next cell the neighbour
                if neighbour_score < next_cell_score and neighbour != previous_cell:
                    next_cell = neighbour
        ...
        # Add the current cell to the path and return it
        path.append(current_cell)
...
for cell in path:
    length += grid.grid[cell]

for cell in path:
    grid.display_grid(cell)
    time.sleep(0.25 * grid.grid[cell])
